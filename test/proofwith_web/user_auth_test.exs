defmodule ProofwithWeb.UserAuthTest do
  use ProofwithWeb.ConnCase

  import Proofwith.AccountsFixtures

  alias Phoenix.LiveView
  alias Phoenix.Socket.Broadcast
  alias Proofwith.Accounts
  alias Proofwith.Accounts.Scope
  alias ProofwithWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ProofwithWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ProofwithWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ProofwithWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
